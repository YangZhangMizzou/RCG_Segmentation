...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='pascal_voc',
                        choices=['rcg','pascal_voc', 'pascal_aug', 'ade20k', 'citys', 'sbu'],
    ...
    parser.add_argument('--save-dir', default='../checkpoint/models',
...
class Trainer(object):
    def __init__(self, args):
        ...
        test_dataset = get_segmentation_dataset(args.dataset, split='test', mode='val', **data_kwargs)
    ...
    def train(self):
        ...
        self.final_test(self.train_loader)
        self.final_test(self.val_loader)
        self.final_test(self.test_loader)
        logger.info(
    ...
    def validation(self):
        ...
        logger.info(" Validation pixAcc: {:.3f}, f1_score: {:.3f}".format(pixAcc, mIoU))
        new_pred = get_average(mIoU_list)
    ...
    def final_test(self,loader_type):
        # total_inter, total_union, total_correct, total_label = 0, 0, 0, 0
        self.metric.reset()
        if self.args.distributed:
            model = self.model.module
        ...
        for i, (image, target, filename) in enumerate(loader_type):
            image = image.to(self.device)
            target = target.to(self.device)
        ...
        logger.info(" Test pixAcc: {:.3f}, f1-score: {:.3f}".format(pixAcc, mIoU))

        pixAcc_list = []
        mIoU_list = []
        for i, (image, target, filename) in enumerate(self.train_loader):
            image = image.to(self.device)
            target = target.to(self.device)
        ...
        logger.info(" Training pixAcc: {:.3f}, f1-score: {:.3f}".format(get_average(pixAcc_list), get_average(mIoU_list)))


def save_checkpoint(model, args, is_best=False):
    """Save Checkpoint"""
    directory = os.path.expanduser(args.save_dir+'/{}_{}_{}'.format(args.backbone,args.model,args.dataset))
    ...
    if is_best:
        best_filename = '{}_{}_{}_best_model.pth'.format(args.backbone, args.model,  args.dataset)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("semantic_segmentation", args.log_dir, get_rank(), filename='{}_{}_{}_log.txt'.format(
        args.backbone, args.model,  args.dataset))
