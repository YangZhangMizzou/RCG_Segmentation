...
def get_psanet(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='~/.torch/models', local_rank = 0,
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('psanet_%s_%s' % (backbone, 'rcg'), root=root),
                              map_location=device))
    return model
