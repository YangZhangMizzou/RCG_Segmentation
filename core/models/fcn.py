...
def get_fcn32s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('fcn32s_%s_%s' % (backbone, 'rcg'), root=root),
                              map_location=device))
    return model
...
def get_fcn16s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('fcn16s_%s_%s' % (backbone, 'rcg'), root=root),
                              map_location=device))
    return model
...
def get_fcn8s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('fcn8s_%s_%s' % (backbone, 'rcg'), root=root),
                              map_location=device))
    return model
